...
- `app-*.js` (16 archivos) — lógica principal: ranking, estadísticas, panel admin, Batallas. **v8.0** — hasta la v7.x esto era un único `app.js` de 3906 líneas; se dividió en 16 módulos de responsabilidad única (Sprint 1, roadmap de arquitectura), cada uno un slice contiguo y literal del `app.js` anterior, sin cambios de lógica (verificado byte a byte). Cargan en el mismo orden relativo exacto en que estaban dentro del archivo único — ese orden importa: comparten el scope global del navegador entre sí, igual que ya hacían participantes.js/utils.js/scoring.js/totp.js. **`app-bootstrap.js` debe ser siempre el último**: hace el primer render llamando funciones definidas en todos los módulos anteriores. El detalle de qué contiene cada uno:
  - `app-core-data.js` — datos maestros (equipos, grupos, banderas, reglas), arranca PL/PM/MD/MIDS
  - `app-admin-auth.js` — Firebase Auth (admin) + 2FA TOTP + objeto de estado global `S`
