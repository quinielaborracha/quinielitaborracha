...
- `torneo-mundial2026.js` — **la config del torneo en curso**: un solo objeto `TORNEO_MUNDIAL_2026` con el fixture completo del Mundial 2026. `groupMatches` (72 entradas `{id,g,a,b}`) es la fuente ÚNICA de los partidos de grupos — `matchLabels`/`mgmap` se derivan de ahí mismo (antes eran diccionarios paralelos + una tercera copia en `registro.js`, coincidentes por disciplina manual, no por construcción). También trae `espnAbbrMap`/`midAbbrs`/`ges`/`arules`/`espnGameIdToPid`. Sprint 2+3b de la hoja de ruta comercial (2026-07-22): consolida lo que antes vivía repartido entre `partidos-grupos.js`, `app-static-data.js`, `registro.js` y `app-live-sync.js`. Un futuro segundo torneo (Copa América, Euro) trae su propio objeto `TORNEO_<NOMBRE>` en su propio archivo, sin tocar `scoring.js`/`utils.js`/`app-bracket-*.js`/`registro.js`.
- `partidos-grupos.js` — reasigna `MATCH_LABELS` desde `TORNEO_MUNDIAL_2026.matchLabels` (re-export delgado; el dato en sí vive en `torneo-mundial2026.js`). Reemplaza al viejo `legacy-migracion.js` del proyecto original, que sí traía datos personales de 27 participantes — por diseño, **no se incluye acá**.
...
- `app-static-data.js` — reasigna `ESPN_ABBR_MAP`/`MID_ABBRS`/`MGMAP`/`GES`/`ARULES` desde `TORNEO_MUNDIAL_2026` (mismo patrón que `partidos-grupos.js`). `FLAGS2`/`ABBR`/`BGCOL` quedan con su valor literal acá — un set más chico, no específico del fixture del torneo en curso.
